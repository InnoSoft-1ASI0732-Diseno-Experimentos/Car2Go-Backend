...
/**
 * The type Transaction command service.
 */
@Service
public class TransactionCommandServiceImpl implements TransactionCommandService {
  ...
  /**
   * Instantiates a new Transaction command service.
   *
  ...
  public Optional<Transaction> handle(CreateTransactionCommand command) {
    ...
    if (hasRequiredRole) {
      ...
      Vehicle vehicle =
          vehicleQueryService.handle(new GetVehicleByIdQuery(Math.toIntExact(command.vehicleId())))
              .orElseThrow(() -> new IllegalArgumentException("Vehicle not found"));
      Profile profileSeller =
          profileQueryService.handle(new GetProfileByIdQuery(vehicle.getProfileId()))
