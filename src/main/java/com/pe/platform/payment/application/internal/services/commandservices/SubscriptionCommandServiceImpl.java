...
/**
 * The type Subscription command service.
 */
@Service
public class SubscriptionCommandServiceImpl implements SubscriptionCommandService {
  ...
  /**
   * Instantiates a new Subscription command service.
   *
  ...
  public Optional<Subscription> handle(CreateSubscriptionCommand command) {
    Optional<Subscription> existingSubscription =
        subscriptionRepository.findByProfileId(command.profileId());
    if (existingSubscription.isPresent()) {
      throw new IllegalStateException("A subscription already exists for this user");
    }

    var subscription =
        new Subscription(command.price(), command.description(), SubscriptionStatus.PAID,
            command.profileId());
