...
public class SubscriptionController {
  ...
  /**
   * Create subscription response entity.
   *
  ...
  public ResponseEntity<SubscriptionResource> createSubscription(
    ...
    var command =
        new CreateSubscriptionCommand(resource.price(), resource.description(), profileId);

    Optional<Subscription> subscription = subscriptionCommandService.handle(command);
    return subscription.map(resp ->
            new ResponseEntity<>(SubscriptionResourceFromEntityAssembler.toResourceFromEntity(resp),
                CREATED))
  ...
  /**
   * Cancel my subscription response entity.
   *
   * @return the response entity
   */
  @PreAuthorize("hasRole('ROLE_SELLER')")
  ...
  public ResponseEntity<SubscriptionResource> cancelMySubscription() {
    ...
    Optional<Subscription> subscription = subscriptionCommandService.handle(
        new UpdateSubscriptionCommand(profileId, SubscriptionStatus.CANCELLED));
    return subscription.map(
            sub -> ResponseEntity.ok(SubscriptionResourceFromEntityAssembler.toResourceFromEntity(sub)))
        .orElseGet(() -> ResponseEntity.notFound().build());
  }
